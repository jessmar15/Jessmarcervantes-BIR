...
?>
  ...
  <div class="row">
    <div class="col">
        <button type="button" class="btn btn-primary">Time In</button>
        <a href="indexout.php"><button type="button" class="btn btn-secondary">Time Out</button></a>
        <br><br>
        <h4>Time In</h4>
            <form  method="post" action="">
                <div class="form-group">
                    <label for="rfidInput">RFID Card:</label>
                    <input type="text" name="rfid_card_timein" class="form-control" id="myTextboxIn" autocomplete="off">
                </div>
            </form>
    </div>
  </div>
...
<?php
// Time In
if (isset($_POST['rfid_card_timein'])) {
    $rfid_card = $_POST['rfid_card_timein'];

    if (empty($rfid_card)) {
        die("RFID card value is required.");
    }

    // Step 1: Find the employee with the given RFID card
    ...
    if ($employee) {
        ...
        // Step 2: Check if an entry already exists for the same rfid_card and date
        $checkStmt = $conn->prepare('
            SELECT COUNT(*) FROM timein WHERE rfid_card = :rfid_card AND date = :date
        ');
        ...
        if ($existingEntries == 0) {
            // Step 3: Insert the date, time, and rfid_card into the timein table
            $insertStmt = $conn->prepare('
                INSERT INTO timein (rfid_card, date, time_in)
                VALUES (:rfid_card, :date, :time_in)
            ');
